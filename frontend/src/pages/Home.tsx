              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {(top.data?.coins ?? []).map((c) => (
                  <Link
                    key={c.id}
                    to={`/tokens/${c.id}`}
                    className="flex items-center gap-2 rounded-lg border border-white/[0.06] bg-card/30 p-2 backdrop-blur-sm transition-all duration-200 hover:border-primary/20 hover:bg-card/50 sm:gap-3"
                  >
          ...
          <CardContent className="space-y-3">
            <QueryState isLoading={gas.isLoading} error={gas.error}>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg border border-white/[0.06] bg-card/30 p-2">
                ...
                <div className="rounded-lg border border-primary/30 bg-primary/5 p-2">
                  ...
                  <div className="num text-base font-semibold text-primary">{gas.data?.propose ?? "-"}</div>
                </div>
                <div className="rounded-lg border border-white/[0.06] bg-card/30 p-2">
          ...
          {hubs.map((s, idx) => {
            ...
            return (
              <Link
                ...
                className="fade-rise group relative flex h-full flex-col overflow-hidden rounded-xl border border-white/[0.06] bg-card/40 p-4 backdrop-blur-sm transition-all duration-300 hover:border-primary/25 hover:bg-card/60 hover:shadow-lg hover:shadow-primary/[0.04]"
              >
